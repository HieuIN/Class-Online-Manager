        ...
        <template v-if="activeCharacter">
          ...
          <div v-else class="practice-card">
            <div class="practice-copy">
              ...
              <small class="input-help">Dùng chuột, màn hình cảm ứng hoặc bút cảm ứng để viết từng nét</small>
            </div>
            <div ref="writerWrap" class="writer-wrap"><div ref="writerTarget" class="writer-target"></div></div>
...
const showSet=ref(false), showCharacter=ref(false), editingId=ref(null), uploading=ref(false), writerTarget=ref(null), writerWrap=ref(null);
...
function makeWriter(){if(!writerTarget.value||!activeCharacter.value)return;writerTarget.value.innerHTML='';const size=Math.max(240,Math.min(360,Math.floor(writerWrap.value?.clientWidth||320)));writer=HanziWriter.create(writerTarget.value,activeCharacter.value.character,{width:size,height:size,padding:Math.round(size*.055),showOutline:true,showCharacter:false,strokeColor:'#1b2b26',outlineColor:'#d9e5df',drawingColor:'#16856f',drawingWidth:Math.max(8,Math.round(size/32))});}
...
.input-help{margin-top:7px}.writer-wrap{aspect-ratio:1;background:linear-gradient(45deg,transparent 49.7%,#eef2f0 49.7% 50.3%,transparent 50.3%),linear-gradient(-45deg,transparent 49.7%,#eef2f0 49.7% 50.3%,transparent 50.3%),linear-gradient(0deg,transparent 49.7%,#e0e8e4 49.7% 50.3%,transparent 50.3%),linear-gradient(90deg,transparent 49.7%,#e0e8e4 49.7% 50.3%,transparent 50.3%);height:auto;max-width:360px;overflow:hidden;touch-action:none;width:100%;-webkit-user-select:none;user-select:none}.writer-target{height:100%;touch-action:none;width:100%}.writer-target :deep(svg),.writer-target :deep(canvas){display:block;touch-action:none}@media(max-width:800px){.content-panel :deep(.el-card__body){padding:14px}.practice-actions{align-items:stretch;flex-direction:column}.practice-actions .el-button{margin-left:0;width:100%}.manage-actions{flex-wrap:wrap}}@media(max-width:420px){.writer-wrap{max-width:calc(100vw - 64px)}.hanzi{font-size:76px}.mode-tabs{display:flex}.mode-tabs .el-button{flex:1;margin-left:0}}
</style>
